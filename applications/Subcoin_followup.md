...
This project is planned as 2 milestones, completed in three months.
...
| 1. | Full Bitcoin Fast Sync | A public bootstrap node synced to Bitcoin’s tip, enabling users to quickly sync to Bitcoin’s latest state using fast sync. |
...
| 1. | `subcoin-snapcake` | We will provide a dedicated tool for downloading the UTXO set snapshot compatible with Bitcoin Core from the Subcoin P2P network. This tool does not actually a full regular Substrate node but only utilizes the Substate sync compoenents, thus it’s pretty resources-efficient. |
